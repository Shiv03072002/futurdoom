...
const GroupChat = () => {
  ...
  // Group info - Get from selected users or use default
  const [groupInfo, setGroupInfo] = useState({
    id: Date.now(),
    name: "New Group",
    avatar: "NG",
    createdBy: "Shiv", // Current user who created the group
    createdAt: new Date().toLocaleString(),
    members: [
      { id: "current", name: "Shiv", avatar: "SH", color: "from-blue-400 to-indigo-500", isCreator: true, online: true },
      ...selectedUsers.map((userId, index) => {
        // Map selected user IDs to actual user objects (you'd get this from your users list)
        const userMap = {
  ...
  // Initial system message showing who created the group
  useEffect(() => {
    if (groupInfo.members.length > 0) {
      ...
      const welcomeMessage = {
        id: "welcome-1",
        type: "system",
        text: `👋 Welcome ${memberNames ? memberNames + " and " : ""}${creatorName}! Say hello to the group.`,
        time: "Just now",
      };
  ...
  const send = () => {
    ...
    // Simulate group member typing
    setIsTyping(true);
    setTimeout(() => {
      setIsTyping(false);
      // Randomly select a group member to reply
      const otherMembers = groupInfo.members.filter(m => !m.isCreator);
      if (otherMembers.length > 0) {
        const randomMember = otherMembers[Math.floor(Math.random() * otherMembers.length)];
        const replies = [
          `Hey ${randomMember.name} here! 👋`,
          "Great to be in this group!",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-0.5 sm:gap-1">
                {[
                  ...
                  { Icon: Info, label: "Info", action: () => setShowMembers(!showMembers) },
            ...
            {/* Members Panel (when Info is clicked) */}
            <AnimatePresence>
              {showMembers && (
                <motion.div
                    ...
                    >
                      ...
                      <div className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`flex flex-col max-w-[85%] sm:max-w-[70%] ${isMe ? 'items-end' : 'items-start'}`}>
                          {/* Sender name for group messages (visible for non-me messages) */}
                          {!isMe && (
