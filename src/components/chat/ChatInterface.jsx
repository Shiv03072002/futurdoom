...
//         className="absolute right-0 top-full mt-2 w-36 bg-white rounded-xl border border-gray-200 shadow-lg z-50 overflow-hidden"
...
const ChatInterface = ({ messages: propMessages, currentChatId, onMessagesUpdate }) => {
  ...
  return (
    // ✅ h-full — fills whatever height ChatMain gives it, no fixed height
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="h-full"
    >
      <div
        className="flex flex-col overflow-hidden bg-white rounded-xl h-full "
        style={{ border: "1px solid #dbeafe" }}
      >

        {/* Header — opacity fade only, no y-slide (avoids clipping on mobile) */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex items-center gap-2 sm:gap-3 flex-shrink-0 px-3 sm:px-5 py-2.5 sm:py-3 relative"
        ...
        {/* Messages — flex-1 + min-h-0 makes this area scroll instead of pushing input away */}
        <div
          ref={messagesContainerRef}
        ...
        {/* Input bar — flex-shrink-0 pins it to bottom always */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 px-2 sm:px-4 py-2.5 sm:py-3 bg-white relative"
